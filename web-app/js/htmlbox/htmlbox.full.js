...
$.fn.htmlbox=function(options){
		...
		for(var k=0;k<d.toolbars.length;k++){
			...
			for(var i=0;i<(toolbar.length);i++){
	            ...
	            // END: Custom button
				    ...
				    h += "<td valign='middle' align='center'><select id='"+d.id+"_highlight' onchange='global_hb[\""+d.id+"\"].cmd(\"backcolor\",this.options[this.selectedIndex].value)' style='font-size:12px;'><option value='' selected>-SURBRILLANCE-</option>";
	...
	this.cmd = function(cmd,arg1){
	   ...
	   }else if(cmd==="etablissement"){
		   d.idoc.execCommand("insertHTML", false, "[ETAB]<br/>");
	   }else if(cmd==="patient"){
		   d.idoc.execCommand("insertHTML", false, "[PAT]");
	   }else if(cmd==="patientcomp"){
		   d.idoc.execCommand("insertHTML", false, "[PAT-COMP] ");
	   }else if(cmd==="medecin"){
		   d.idoc.execCommand("insertHTML", false, "[MED] ");
	   }else if(cmd==="signature"){
		   d.idoc.execCommand("insertHTML", false, "[SIGNATURE] ");
	   }else if(cmd==="date"){
		   d.idoc.execCommand("insertHTML", false, "[DATE] ");
	   }else if(cmd==="datelieu"){
		   d.idoc.execCommand("insertHTML", false, "[DATELIEU] ");
	   }else if(cmd==="champs"){
		   d.idoc.execCommand("insertHTML", false, "[CTEXT] ");
	   }else if(cmd==="zone"){
		   d.idoc.execCommand("insertHTML", false, "<br/><br/>[ZTEXT]<br/><br/> ");
	   }else if(cmd==="liste"){
		   d.idoc.execCommand("insertHTML", false, "[LISTE] ");
	   }else if(cmd==="link"){
